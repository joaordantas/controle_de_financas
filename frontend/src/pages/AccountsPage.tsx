import { ArrowDownLeft, ArrowRight, ArrowRightLeft, Landmark, Plus, WalletCards } from "lucide-react";
...
export function AccountsPage() {
  ...
  const totalBalance = useMemo(
    () => accounts.reduce((total, account) => total + account.saldo_atual, 0),
    [accounts],
  );
  ...
  async function load() {
    ...
    try {
      ...
      setSourceId((current) => current || accountData[0]?.id || 0);
      setDestinationId((current) => current || accountData[1]?.id || 0);
      setError("");
    } catch (err) {
  ...
  return (
    <div className="page-stack">
      <PageHeader
        description="Acompanhe onde está seu dinheiro e mova valores entre contas."
        eyebrow="Patrimônio disponível"
        title="Contas"
      />
      ...
      <Card className="accounts-total-card">
        <span>Saldo total nas contas</span>
        <strong>{loading ? "—" : formatCurrency(totalBalance)}</strong>
        <small>Soma do saldo inicial e das movimentações vinculadas</small>
      </Card>
      ...
      <section className="account-cards" aria-label="Contas financeiras">
        ...
        {!loading && accounts.length === 0 ? (
          <Card className="accounts-empty-card">
            <EmptyState description="Adicione onde seu dinheiro está para acompanhar o saldo corretamente." icon={Landmark} title="Você ainda não possui contas" />
          </Card>
        ) : null}
        {!loading && accounts.map((account) => (
          <Card className="account-card" key={account.id}>
            ...
            <div>
              <small>{accountTypeLabels[account.tipo]}</small>
              <h2>{account.nome}</h2>
            </div>
      ...
      <div className="accounts-layout">
        <Card as="section" className="account-form-card">
          ...
          <form className="form-grid" onSubmit={handleCreateAccount}>
            <label>Nome<input onChange={(event) => setName(event.target.value)} placeholder="Ex.: Nubank" value={name} /></label>
        ...
        <Card as="section" className="transfer-form-card">
          ...
          {accounts.length < 2 ? (
            <div className="inline-empty"><ArrowRightLeft size={20} /><p>Adicione pelo menos duas contas para fazer uma transferência.</p></div>
          ) : (
            <form className="form-grid" onSubmit={handleTransfer}>
              <div className="transfer-accounts">
                <label>De<select onChange={(event) => setSourceId(Number(event.target.value))} value={sourceId}>{accounts.map((account) => <option disabled={account.id === destinationId} key={account.id} value={account.id}>{account.nome}</option>)}</select></label>
                <ArrowRight aria-hidden="true" size={20} />
                <label>Para<select onChange={(event) => setDestinationId(Number(event.target.value))} value={destinationId}>{accounts.map((account) => <option disabled={account.id === sourceId} key={account.id} value={account.id}>{account.nome}</option>)}</select></label>
              </div>
              ...
              <label>Descrição opcional<input onChange={(event) => setDescription(event.target.value)} placeholder="Ex.: Dinheiro para pagamentos" value={description} /></label>
              <label>Data<input onChange={(event) => setTransferDate(event.target.value)} type="date" value={transferDate} /></label>
              <Button disabled={saving || transferValue <= 0 || sourceId === destinationId} type="submit"><ArrowRightLeft size={18} />Transferir</Button>
            </form>
          )}
        </Card>
      </div>
      ...
      <Card as="section" className="transfer-history-card">
        ...
        {transfers.length === 0 ? (
          <EmptyState description="Transferências entre suas contas aparecerão aqui." icon={ArrowRightLeft} title="Nenhuma transferência registrada" />
        ) : (
          <div className="transaction-list">
            {transfers.map((transfer) => (
              <div className="transaction-row" key={transfer.id}>
                <span className="transaction-icon transfer"><ArrowDownLeft size={18} /></span>
                <span className="transaction-info"><strong>{transfer.conta_origem} → {transfer.conta_destino}</strong><small>{transfer.descricao || "Transferência"} · {formatDate(transfer.data)}</small></span>
                <strong>{formatCurrency(transfer.valor)}</strong>
              </div>
            ))}
          </div>
        )}
